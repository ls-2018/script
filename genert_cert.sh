...
old_func() {
    ...
    # 2、生成证书请求
    openssl req -new -sha256 -key flask.key -out flask.csr -subj /C=CN/ST=Beijing/L=Beijing/O=Devops/CN=flask-sidecar-injector
    ...
    #2、制作自签证书# -------可以使用域名访问myapp.magedu.com--------------
    # 如果是自己做测试，那么证书的申请机构和颁发机构都是自己。就可以用下面这个命令来生成证书：
    # openssl req -new -x509 -key flask.key -out flask.crt -subj /C=CN/ST=Beijing/L=Beijing/O=Devops/CN=myapp.magedu.com -days 360000
