...
docker run -d -v /Volumes/Tf/data/registry:/var/lib/registry \
	...
	-p 5000:5000 --restart=always --name simple_registry $(trans_image_name.py docker.io/library/registry)
...
docker run -p 8280:80 --restart=always --name simple_registry-ui \
	...
	-d $(trans_image_name.py docker.io/joxit/docker-registry-ui:1.5-static)
