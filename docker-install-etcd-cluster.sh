...
docker run -d --name etcd0 --network etcd --ip 172.19.1.10 -P $(trans-image-name registry.k8s.io/etcd:3.5.2) etcd \
	-name etcd0 \
...
docker run -d --name etcd1 --network etcd --ip 172.19.1.11 $(trans-image-name registry.k8s.io/etcd:3.5.2) etcd \
	-name etcd1 \
...
docker run -d --name etcd2 --network etcd --ip 172.19.1.12 $(trans-image-name registry.k8s.io/etcd:3.5.2) etcd \
	-name etcd2 \
